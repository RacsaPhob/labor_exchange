...
Репозитории **SyncHTTPRepository** и **AsyncHTTPRepository** реализованы в соответствии с интерфейсом **BaseRepository**. Каждый метод CRUD соответствует HTTP-методу. Метод `list()` остается неопределённым.
